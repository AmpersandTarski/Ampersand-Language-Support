...
export class ampersandParser {
    static parseAmpersandOutput(
        ...
        function split(xs: string[]): string[][] {
            ...
            let res = [];
            for (let x of xs) {
                if (isMessageBody(x)) cont.push(x);
                else {
        ...
        function parse(xs: string[]): [vscode.Uri, vscode.Diagnostic][] {
            ...
            let f = (l1: number, c1: number, l2: number, c2: number) => {
                ...
                let file = vscode.Uri.file(
                    path.isAbsolute(m[1]) ? m[1] : path.join(dir, m[1])
                );
            ...
            if (xs[0].startsWith("All good")) return [];
            if ((m1 = xs[0].match(r1))) {
                m = m1;
                return f(2, 3, 2, 3);
                ...
                return [
                    [
                        vscode.Uri.file(""),
                        new vscode.Diagnostic(new vscode.Range(0, 0, 0, 0), xs.join("\n")),
                    ],
                ];
            }
        }
